...
public class CPHInline
{
    ...
    public bool Execute()
    {
        ...
        if (!IsRequestAllowed(1, user, false))
            return false;
        if (string.IsNullOrWhiteSpace(rawInput))
        {
            SendYouTubeMessage("Usage: !translate [lang_code] [markers] <text>");
            return false;
        }

        if (IsInputBlocked(rawInput, user))
            return false;
                ...
                {
                    {
                        "she/her",
                        "[ella]"
                    }, // Same as Italian, a translated "amiga" is enough.
                    {
                        "he/him",
                        "[él]"
                    },
                    {
                        "they/them",
                        "[elle]"
                    } // Spanish uses "-e" endings, so a marker is optional.
                }
            }
        };
        ...
        if (!IsRequestAllowed(requiredApiCallCount, user, true))
            return false;
        ...
        catch (Exception ex)
        {
            ...
            SendYouTubeMessage($"Sorry @{user}, a bot error occurred.");
        }
...
- CRITICAL: You MUST KEEP the placeholders like [P1], [P2] in your final translated response exactly as they are.";
        ...
        if (properNouns.Any())
        {
            properNounInstruction = $"PROPER NOUNS: The following words are proper nouns and MUST NOT be translated: [{string.Join(", ", properNouns)}].";
        }

        string toneInstruction = "";
        if (!string.IsNullOrEmpty(tone) && tone != "neutral")
        {
            toneInstruction = $"TONE INSTRUCTION: The final translation MUST be delivered in a '{tone}' tone.";
        }

        string jokeInstruction = "";
        if (targetLanguage == "incoherent baby babble" || targetLanguage == "Donkey Kong style ape sounds")
        {
            jokeInstruction = "The length of your response MUST be proportionate to the input text.";
        }
...
{text}";
    ...
    private bool IsInputBlocked(string rawInput, string user)
    {
        try
        {
            string blocklistStr = CPH.GetGlobalVar<string>("translateBlocklist", true);
            if (!string.IsNullOrEmpty(blocklistStr) && blocklistStr.Split(',').Select(s => s.Trim().ToLower()).Any(word => !string.IsNullOrEmpty(word) && rawInput.ToLower().Contains(word)))
            {
                SendYouTubeMessage($"Sorry @{user}, that message cannot be translated.");
    ...
    private void SendLongYouTubeMessage(string message)
    {
        if (string.IsNullOrEmpty(message))
            return;
        if (message.Length <= YOUTUBE_CHAR_LIMIT)
        {
        ...
        {
            int maxChunkLength = YOUTUBE_CHAR_LIMIT - 7;
        ...
        {
            SendYouTubeMessage($"({i + 1}/{chunks.Count}) {chunks[i]}");
            CPH.Wait(500);
        }
    }

    private void SendYouTubeMessage(string message)
    {
    ...
    private string PerformApiCall(HttpClient client, string url, string prompt)
    {
        ...
        {
            throw new Exception("API call failed: " + response.Content.ReadAsStringAsync().Result);
        }

        string jsonResponse = response.Content.ReadAsStringAsync().Result;
        JObject parsedResponse = JObject.Parse(jsonResponse);
        return parsedResponse["candidates"][0]["content"]["parts"][0]["text"].ToString().Trim().Trim('"');
    }

    private bool IsRequestAllowed(int requestCount, string user, bool isFinalCheck)
    {
        ...
        {
            dailyCount = 0;
            CPH.SetGlobalVar("geminiLastRequestDate", today, true);
            CPH.SetGlobalVar("geminiRequestCountDaily", 0, true);
        }

        if (dailyCount + requestCount > MAX_REQUESTS_PER_DAY)
        {
            SendYouTubeMessage($"@{user}, that command is too complex for the remaining daily API limit.");
            return false;
        }
        ...
        if (!string.IsNullOrEmpty(timestampsStr))
        {
            recentTimestamps = timestampsStr.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries).Select(long.Parse).Where(ts => ts > sixtySecondsAgo).ToList();
        }

        if (recentTimestamps.Count + requestCount > MAX_REQUESTS_PER_MINUTE)
        {
            SendYouTubeMessage($"@{user}, that command is too complex for the current rate limit. Please try again in a moment!");
            return false;
        }

        if (isFinalCheck)
        {
            CPH.SetGlobalVar("geminiRequestCountDaily", dailyCount + requestCount, true);
